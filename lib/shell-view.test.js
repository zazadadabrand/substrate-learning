...
test('materials are one atom per call and commissioned tracks stay on the shell', () => {
  const html = fs.readFileSync(new URL('../index.html', import.meta.url), 'utf8');
  const api = fs.readFileSync(new URL('../api/materials.js', import.meta.url), 'utf8');
  assert.equal(html.includes('id="gtrackView"'), false);
  assert.equal(html.includes('function renderGTrack'), false);
  assert.equal(html.includes('function openGTrack'), false);
  assert.match(html, /await pullModules\(t, layer, \[item\]\)/);
  assert.match(api, /const MAX_ATOMS = 1/);
  assert.match(api, /const MAX_TOKENS = 2048/);
  assert.match(api, /const UPSTREAM_MS = 45000/);
  assert.equal(api.includes('max_tokens: 8192'), false);
  assert.equal(api.includes('compact: true'), false);
});
